...
def get_data_series_list(
    ...
    for i in range(len(metric_id_list)):
        data_series = {
            "metric_id": checking("metrics", metric_id_list[i]),
...
def main():
    ...
    parser.add_argument(
        "--initial_date", required=True, type=valid_date, help="Format YYYY-MM-DD"
    )
    parser.add_argument(
        "--final_date", required=True, type=valid_date, help="Format YYYY-MM-DD"
    )
    parser.add_argument(
        "--groapi_token",
        type=str,
        default=os.environ["GROAPI_TOKEN"],
        help="GroAPI token",
    )
    parser.add_argument(
        "--api_host", type=str, default="api.gro-intelligence.com", help="api host"
    )
    ...
    parser.add_argument(
        "--start_date",
        type=valid_date,
        help="start date of all the Gro data " "series to be used for this " "analysis",
    )
    parser.add_argument(
        "--ENSO",
        action="store_true",
        help="Include ENSO for rank distance " "calculation",
    )
    parser.add_argument(
        "--ENSO-weight", type=float, default=1, help="Weight of the ENSO index"
    )
    parser.add_argument(
        "--all_ranks",
        action="store_true",
        help="Lets you see all the ranks" "as opposed to separate method" "ranks",
    )
    parser.add_argument(
        "--num_jobs",
        type=int,
    ...
    data_series_list = get_data_series_list(
        args.region_id,
        args.item_ids,
        args.metric_ids,
        args.source_ids,
    ...
    result = final_ranks_computation.analogous_years(
        ...
        enso=args.ENSO,
        enso_weight=args.ENSO_weight,
        provided_start_date=args.start_date,
