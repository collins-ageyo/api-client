...
@mock.patch(
    "api.client.gro_client.GroClient.get_df",
    return_value=create_test_data_for_get_data(),
)
def test_get_data(test_data_1):
    ...
    test_data = get_transform_data.get_data(
        client,
        "metric_id",
        "item_id",
        "region_id",
...
def test_combine_subregions_with_subregion():
    ...
    expected_subregion = pd.DataFrame(
        {
            "end_date": [
                "2000-03-01T00:00:00.000Z",
                "2005-08-28T00:00:00.000Z",
            ...
            "value": [2.39664378851418, 2.28192643351167, 0.13002748115958],
        }
    )
    utc_tz = False
    ...
    expected_subregion.loc[:, "end_date"] = pd.to_datetime(
        expected_subregion["end_date"], utc=utc_tz
    )
    ...
    assert_frame_equal(
        get_transform_data.combine_subregions(test_data_subregion), expected_subregion
    )
...
def test_extract_time_periods_by_dates_with_non_unique_dates():
    ...
    expected_data.loc[:, "period"] = [
        "2019-03-01 to 2019-07-31",
    ...
    expected_data = expected_data.sort_values(
        by=list(expected_data.columns), axis=0, ascending=False
    )
    assert_frame_equal(
        get_transform_data.extract_time_periods_by_dates(
            test_data, initial_date, final_date
        ),
        expected_data,
    )
...
def test_stack_time_periods_by_ddmm_unique_dates():
    ...
    expected = pd.DataFrame(
        {
            "mm-dd": ["03-01", "07-31", "08-28"],
            "2000-01-01 to 2000-12-31": [2.39664378851418, np.nan, np.nan],
            "2005-01-01 to 2005-12-31": [np.nan, np.nan, 1.17640700229636],
            "2019-01-01 to 2019-12-31": [np.nan, 0.13002748115958, np.nan],
        }
    ).set_index("mm-dd")
    expected.columns.name = "period"
    assert_frame_equal(
        get_transform_data.stack_time_periods_by_ddmm(test_data), expected
    )


def test_dates_to_period_string():
    test_dates = ["2019-07-31T00:00:00.000Z", "2019-08-28T00:00:00.000Z"]
    test_dates = pd.to_datetime(test_dates)
    expected = "2019-07-31 to 2019-08-28"
    assert (
        get_transform_data.dates_to_period_string(test_dates[0], test_dates[1])
        == expected
    )
...
def test_loop_initiation_dates():
    ...
    if test_max_date.tzinfo:
        utc_tz = True
    expected = {
        "initial_date": pd.to_datetime("2017-12-31", utc=utc_tz),
