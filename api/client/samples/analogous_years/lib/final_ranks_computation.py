...
def common_start_date(client, data_series, provided_start_date=None):
    ...
    for i in range(len(data_series)):
        ...
        if len(dates) == 0:
            msg = "No data found for the following gro-data_series - {}".format(
                data_series[i]
            )
            logger.warning(msg)
            raise Exception
...
def get_file_name(
    api_token, data_series_list, initial_date, final_date, api_host=DEFAULT_API_HOST
):
    ...
    logger.info(
        "\n Computing analogous years' ranks in {} with respect to {} between {} and {} \n".format(
            key_words[0], key_words[1:-2], initial_date, final_date
        )
    )
...
def time_series(client, data_series, initial_date, final_date):
    ...
    try:
        ts = get_transform_data.extract_time_periods_by_dates(
            data, initial_date, final_date
        )
        return ts
    except Exception as e:
        message = "Please check availability of data for {}".format(
            client.lookup("items", data_series["item_id"])["name"]
        )
...
def ranked_df_dictionary(
    ...
    dictionary_of_methods = {
        "cumulative_" + item: ay_cumulative,
...
def analogous_years(
    ...
    combined_items_distances.loc[:, "composite"] = combined_items_distances.sum(
        axis=1, skipna=True
    )
    ranks = []
...
def generate_correlation_scatterplots(
    api_token, dataframe, folder_name, output_dir="", api_host=DEFAULT_API_HOST
):
    client = GroClient(api_host, api_token)
    logger = client.get_logger()
