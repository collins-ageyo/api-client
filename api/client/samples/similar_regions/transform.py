...
class FourierCoef(Transformer):
    ...
    def transform(self, time_series):
        ...
        ranges = np.r_[
            0, self.start_idx : self.start_idx + self.num_features - 1
        ].astype(int)
...
def post_process_timeseries(
    ...
    dataset_imputed, coverage = _impute(
        num_of_points, period_length_days, start_datetime, time_series
    )
...
def _fill_in_blank_days(num_of_points, start_datetime, pulled_dataset):
    ...
    for day_idx in range(0, num_of_points):
        ...
        if (
            ptr_idx_incomplete_list >= len(pulled_dataset)
            or pulled_dataset[ptr_idx_incomplete_list]["start_date"] != curr_idx_date
        ):
...
def _impute(num_of_points, period_length_days, start_datetime, pulled_dataset):
    ...
    for datapoint in pulled_dataset:
        if datapoint["value"] is not None:
            delta = (
                datetime.strptime(datapoint["start_date"][0:10], "%Y-%m-%d").date()
                - start_datetime.date()
            ).days
