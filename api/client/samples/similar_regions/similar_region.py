...
class SimilarRegion(object):
    ...
    def _regions_avail_for_selection(self, region_properties):
        ...
        for props in region_properties.values():
            for available_series in self.client.list_available(
                props["selected_entities"]
            ):
                regions.add(available_series["region_id"])
        self._logger.info(
            "{} regions are available for comparison.".format(len(regions))
        )
        return list(regions)
    ...
    def _format_results(self, sim_regions, requested_region_level, dists, metric_dists):
        for ranking, sim_region_region_id in enumerate(sim_regions):
            ...
            if (
                requested_region_level is not None
                and region_level != requested_region_level
            ):
                self._logger.info(
                    "not level %s %s" % (requested_region_level, sim_region_name)
                )
                continue
            ...
            if parent.get("id", None):
                grandparent = next(
                    self.client.lookup_belongs("regions", parent["id"]),
                    {"name": "", "id": ""},
                )
            else:
                grandparent = {"name": "", "id": ""}
            self._logger.info(
                u"{}, {}, {}".format(
                    sim_region_name, parent["name"], grandparent["name"]
                )
            )
            metric_dists_dict = {
                prop_name: distance
                for (prop_name, distance) in zip(
                    self.state.region_properties.keys(), metric_dists[ranking]
                )
            }
            data_point = {
                "id": sim_region_region_id,
                "name": sim_region_name,
                "dist": dists[ranking],
                "parent": (
                    parent["id"],
                    parent["name"],
    ...
    def _similar_to(self, region_id, number_of_regions):
        ...
        :return: regions that are most similar to the given region id, the distances to each of these, and the 
                    ...
                    dictionary). 
        """
        assert region_id in self.state.mapping, (
            "This region is not available in your configuration or "
        ...
        x = x.reshape(1, -1)
        neighbour_dists, neighbour_idxs = self.ball.query(
            x, k=min(number_of_regions, self.state.num_regions)
        )

        # get the individual distances
        metric_distances = [
            self._get_distances(self.state.mapping[region_id], idx2)
            for idx2 in neighbour_idxs[0]
        ]

        return (
            self.state.inverse_mapping[neighbour_idxs[0]],
            neighbour_dists[0],
            metric_distances,
        )
    ...
    def _get_distances(self, idx1, idx2):
        ...
        for properties in self.state.region_properties.values():
            ...
            subspace_vec1 = self.state.data_standardized[
                idx1, progress_idx : progress_idx + num_features
            ]
            subspace_vec2 = self.state.data_standardized[
                idx2, progress_idx : progress_idx + num_features
            ]
    ...
    def similar_to(self, region_id, number_of_regions=10, requested_level=None):
        ...
        """
        sim_regions, dists, metric_dists = self._similar_to(
            region_id, number_of_regions
        )
        self._logger.info(
            "Found {} regions most similar to '{}'.".format(len(sim_regions), region_id)
        )
        for output in self._format_results(
            sim_regions, requested_level, dists, metric_dists
        ):
            yield output
