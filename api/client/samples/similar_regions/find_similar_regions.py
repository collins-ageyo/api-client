...
def main():
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="total number of regions to query from the region data set",
    )
    parser.add_argument(
        "--data_dir",
        default=None,
        type=str,
        help="directory to store cached data in. by default in a tmp dir or in a special directory in local dir",
    )
    parser.add_argument(
        "--no_download",
        action="store_true",
        help="don't download any new region data; use only cached data",
    )
    args = parser.parse_args()

    if args.csv_output:
        level_suffix = (
            "" if not args.region_level else "_level_" + str(args.region_level)
        )
        f = open(str(args.region_id) + level_suffix + ".csv", "wb")
        csv_writer = None

    sim = SimilarRegion(
        region_properties, data_dir=args.data_dir, no_download=args.no_download
    )
    for output in sim.similar_to(
        args.region_id, args.number_of_regions, args.region_level
    ):
        if args.csv_output:
            if not csv_writer:
