...
class CropModel(GroClient):
    def compute_weights(self, crop_name, metric_name, regions):
        ...
        for region in regions:
            series = self.add_data_series(
                ...
                metric=metric_name,
                region=region["name"],
                result_filter=lambda x: "region_id" not in x
                or x["region_id"] == region["id"],
            )
            if series:
        ...
        def mapper(region):
            return df[
                (df["item_id"].isin(entities["item_ids"]))
    ...
    def compute_crop_weighted_series(
        self,
        weighting_crop_name,
    ...
    def compute_gdd(
        ...
        tmean = (
            df.loc[
                (df.item_id == tmax_series["item_id"])
                | (df.item_id == tmin_series["item_id"])
            ]
            .groupby(
                ["region_id", "metric_id", "frequency_id", "start_date", "end_date"]
            )
            .mean()
        )
    ...
    def growing_degree_days(
        ...
        try:
            ...
            raise Exception(
                "Can't find data series to compute GDD in region {}".format(region_name)
            )
