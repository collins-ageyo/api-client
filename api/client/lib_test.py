...
def initialize_requests_mocker_and_get_mock_data(
    mock_requests_get,
    mock_data={"data": [{"name": "obj1"}, {"name": "obj2"}, {"name": "obj3"}]},
):
...
def test_lookup_with_numpy(mock_requests_get):
    ...
    assert (
        lib.lookup(MOCK_TOKEN, MOCK_HOST, "items", np.array([12345, 67890]))
        == expected_return
    )

    assert (
        lib.lookup(MOCK_TOKEN, MOCK_HOST, "items", np.array([12345])[0])
        == expected_return["12345"]
    )
...
def test_get_data_series(mock_requests_get):
    ...
    assert (
        lib.get_data_series(MOCK_TOKEN, MOCK_HOST, **selection_dict)
        == mock_data["data"]
    )
...
def test_get_data_points(mock_requests_get):
    list_of_series_format_data = [
        {"series": {}, "data": [["2000-01-01", "2000-12-31", 1]]}
    ]
    ...
    initialize_requests_mocker_and_get_mock_data(
        mock_requests_get, mock_data=list_of_series_format_data
    )
    ...
    assert (
        lib.get_data_points(MOCK_TOKEN, MOCK_HOST, **selection_dict)
        == single_series_format_data
    )
...
def test_search_and_lookup(search_mocked, lookup_mocked):
    ...
    search_and_lookup_result = list(
        lib.search_and_lookup(MOCK_TOKEN, MOCK_HOST, "regions", "test123")
    )
...
def test_lookup_belongs(mock_requests_get, lookup_mocked):
    ...
    lookup_belongs_result = list(
        lib.lookup_belongs(MOCK_TOKEN, MOCK_HOST, "regions", 1)
    )
...
def test_get_source_ranking(mock_requests_get):
    ...
    ranked_sources_list = lib.get_source_ranking(
        MOCK_TOKEN, MOCK_HOST, query_parameters
    )
...
def lookup_mock(MOCK_TOKEN, MOCK_HOST, entity_type, entity_ids):
    ...
    if isinstance(entity_ids, list):
        return {
            str(entity_id): LOOKUP_MAP[entity_type][str(entity_id)]
...
def test_descendant_regions(mock_requests_get, lookup_mocked):
    ...
    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_details=True
    ) == [
        {
            "id": 1,
            "name": "region 1",
    ...
    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_details=False
    ) == [{"id": 1}, {"id": 2}]

    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_historical=True
    ) == [
        {
            "id": 1,
            "name": "region 1",
    ...
    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_historical=False
    ) == [
        {
            "id": 2,
            "name": "region 2",
    ...
    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_historical=True, include_details=True
    ) == [
        {
            "id": 1,
            "name": "region 1",
    ...
    assert lib.get_descendant_regions(
        MOCK_TOKEN, MOCK_HOST, 3, include_historical=False, include_details=False
    ) == [{"id": 2}]
...
def test_get_top(mock_requests_get):
    ...
    assert (
        lib.get_top(MOCK_TOKEN, MOCK_HOST, "items", num_results=3, metric_id=14)
        == mock_response
    )
